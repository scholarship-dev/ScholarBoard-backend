...
export function extractEthnicity(text_body: string): string[] {
  const stripped_text = text_body.replace(/\s/g, "");
  const inclusive_ethnicity: string[] = [];
...
*/
export function cleanTextBody(stringArray: string[]): string[] {
  const clean_strings: string[] = [];
...
*/
export function dateFormat(string: string): string {
...
*/
export function extractGrade(text_body: string): string[] {
  const grades: string[] = [];
...
*/
export function extractDeadline(textBody: string): string {
  let deadline: string;
...
*/
export function extractFunding(textBody: string): string {
  const regex = /[0-9]+,[0-9]{3}/;
  const funding = textBody.match(regex)!;
...
*/
export function extractEducationLevel(text_body: string): string[] {
  const eduLevel: string[] = [];
...
*/
export function extractGPA(text_body: string): number | null {
  const new_str = text_body.replace(/\s/g, "");
  let target_gpa: string;
